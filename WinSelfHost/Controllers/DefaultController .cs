...
namespace WinSelfHost.Controllers
{
    public class DefaultController : ApiController
    {
        ...
        public JsonSerializerSettings Settings => new JsonSerializerSettings { Formatting = Formatting.Indented };
...
<h4>Status: {Enum.GetName(typeof(ServiceControllerStatus), value.Status)} - CanStop: {value.CanStop}</h4>
<a href='restartservice/{value.ServiceName}'>Restart {value.ServiceName}</a><hr />");

            var response = new HttpResponseMessage();
            response.Content = new StringContent(builder.ToString(), Encoding.Default);
        ...
        [HttpGet]
        public IHttpActionResult RestartService(string id)
        {
            var services = ServiceController.GetServices();

            if (services.Any(service => service.ServiceName.Equals(id)))
            {
                var find = services.First(service => service.ServiceName.Equals(id));

                if (find.CanStop)
                    find.Stop();

                find.WaitForStatus(ServiceControllerStatus.Stopped);

                find.Start();

                find.WaitForStatus(ServiceControllerStatus.Running);

                var result = new
                {
                    find.CanPauseAndContinue,
                    find.CanShutdown,
                    find.CanStop,
                    find.DisplayName,
                    find.DependentServices,
                    find.MachineName,
                    find.ServiceName,
                    find.ServicesDependedOn,
                    find.ServiceHandle,
                    Status = Enum.GetName(typeof(ServiceControllerStatus), find.Status),
                    ServiceType = Enum.GetName(typeof(ServiceType), find.ServiceType),
                    find.Site,
                    find.Container
                };
            ...
            return Json(new { message = "Não foi possível encontrar o serviço especificado." });
        }
    }
}
